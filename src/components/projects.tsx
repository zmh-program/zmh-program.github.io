...
function Projects({ data }: ProjectsProps) {
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 max-w-[min(80vw,860px)] mx-auto lg:px-4`}>
...
function Project({ idx, title, description, tags, link, avatar, stars }: ProjectType & {
  idx: number;
}) {
    ...
    <Link href={link} target="_blank">
      ...
      >
        ...
        <div className="bg-background-hover shadow transition-all select-none rounded-full p-1.5 absolute top-4 right-4">
          <img loading={"lazy"} src={`/project/${avatar}`}  alt={title} className="rounded-full w-10 h-10" />
        </div>
